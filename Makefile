...
PRODUCT_TARGETS := $(foreach product,$(SELECTED_PRODUCTS),$(foreach suffix,$(SELECTED_SUFFIXES),$(product)$(suffix)))
...
.DEFAULT_GOAL := build
.PHONY: help clean clean-profile configure reconfigure build test \
	install install-vst2 install-au install-standalone

help:
	@echo 'Thin Make wrapper for the existing CMake build'
	@echo
	@echo 'Usage: make [build] [selectors...] [build dimensions...]'
	@echo
	...
	@echo 'Override with:'
	@echo '  VST3_INSTALL_DIR=/path'
	@echo '  VST2_INSTALL_DIR=/path'
	@echo '  AU_INSTALL_DIR=/path'
	@echo '  APP_INSTALL_DIR=/path'
	@echo '  Quote paths containing spaces. Environment overrides are also accepted.'
ifneq ($(WINDOWS_HOST),)
	@echo '  Windows requires GNU Make with a POSIX shell (MSYS2/Git Bash).'
	@echo '  VST2 is a guessed user-local location; add it to your DAW scan paths if needed.'
endif
	@echo
	@echo 'Set selectors to 1 to enable them (unset or 0 disables them):'
	@echo '  OSIRUS  OSTIRUS  VAVRA  XENIA  NODALRED2X  JE8086'
	@echo 'Formats: VST2  VST3  AU  CLAP  LV2  STANDALONE'
	@echo
	@echo 'Build dimensions:'
	@echo '  CONFIG=Release       CMake configuration (Debug, RelWithDebInfo, ...)'
	@echo '  ARCH=native          universal, native, arm64, x86_64, or another CMake arch'
	@echo '  LTO=0                passed through as gearmulator_ENABLE_LTO'
	@echo '  BUILD_ROOT=path      root containing all Make-managed build profiles'
	@echo '  BUILD_DIR=path       override the current profile build directory'
	@echo '  GENERATOR=Ninja      CMake generator (Xcode remains available explicitly)'
	@echo '  JOBS=N               parallel job limit'
	@echo '  TARGETS="name ..."   explicit additional CMake target(s)'
	@echo
	@echo 'Targets: build (default), configure, reconfigure, clean, clean-profile, test'
	@echo '  clean removes every profile under BUILD_ROOT; clean-profile removes only the selected profile.'
	@echo 'Install existing OsTIrus builds: install (VST3), install-vst2, install-au, install-standalone'
	@echo '  Installs do not build first. macOS destinations under /Library use sudo.'
	@echo '  Standalone and Windows/Linux defaults are user-local.'
	@echo
	@echo 'No product or format is selected by default.'
	@echo 'Build defaults: CONFIG=Release ARCH=native LTO=0 JOBS=12'
	@echo
	@echo 'Examples:'
	@echo '  make OSTIRUS=1 VST2=1 STANDALONE=1'
	@echo '  make OSIRUS=1 VST3=1 AU=1 ARCH=universal CONFIG=Release LTO=1'
	@echo '  make VAVRA=1 CLAP=1'
	@echo '  make OSTIRUS=1 GENERATOR=Xcode  # explicitly use the Xcode generator'
	@echo '  make TARGETS=virusLibTest CONFIG=Debug'
...
build: $(CONFIG_STAMP)
	@if test -z "$(BUILD_TARGETS)"; then \
		...
		$(CMAKE) --build "$(BUILD_DIR)" --config "$(CONFIG)" $(BUILD_PARALLEL) --target $(BUILD_TARGETS); \
	fi
...
test: $(CONFIG_STAMP)
	$(CMAKE) --build "$(BUILD_DIR)" --config "$(CONFIG)" $(BUILD_PARALLEL) --target virusLibTest
	ctest --test-dir "$(BUILD_DIR)" -C "$(CONFIG)" -R '^virusLibTests$$' --output-on-failure
...
define install_artifact
	...
	$(call mac_install_privilege,$(2)) $(CMAKE) -E make_directory $(call shell_quote,$(2))
...
define install_artifact
	@test -n $(call shell_quote,$(2)) || { echo 'No install directory configured.' >&2; exit 2; }
	$(CMAKE) -E make_directory $(call shell_quote,$(2))
	@if test -d $(call shell_quote,$(CURDIR)/bin/plugins/$(CONFIG)/$(1)); then \
		$(CMAKE) -E copy_directory $(call shell_quote,$(CURDIR)/bin/plugins/$(CONFIG)/$(1)) $(call shell_quote,$(2)/$(notdir $(1))); \
	else \
		$(CMAKE) -E copy_if_different $(call shell_quote,$(CURDIR)/bin/plugins/$(CONFIG)/$(1)) $(call shell_quote,$(2)/$(notdir $(1))); \
	fi
endef
endif
